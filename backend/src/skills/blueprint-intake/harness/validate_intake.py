...
Usage:
    ...
    python validate_intake.py <path_to_dossier.md> --json   # machine output
...
# ============================================================================
# Schema constants — Recruitment archetype defaults
# ============================================================================

ARCHETYPE_DEFAULTS = {
    "recruitment": {
        "section_a_words_min": 200,
        "section_a_words_max": 420,
...
def check_citation_format(text: str, report: ValidationReport) -> None:
    ...
    arch = ARCHETYPE_DEFAULTS["recruitment"]
...
def check_section_a(sections: dict, report: ValidationReport) -> None:
    if "A" not in sections:
        return
...
def check_section_b(sections: dict, report: ValidationReport) -> None:
...
def check_section_c(sections: dict, report: ValidationReport) -> None:
...
def check_section_d(sections: dict, report: ValidationReport) -> None:
    ...
    for i, chunk in enumerate(h_chunks):
        ...
        if not re.search(r"\*\*Classification hypothesis:\*\*\s*(Quick Win|Foundation Builder|Big Bet)", chunk):
            report.add_fail("section_d_classification", f"Hypothesis {i+1}", "Missing or invalid Classification hypothesis")

def check_section_e(sections: dict, report: ValidationReport) -> None:
...
def check_section_g(sections: dict, report: ValidationReport) -> None:
...
def validate(path: Path, archetype: str = "recruitment") -> ValidationReport:
    ...
    # Per-section checks
    check_section_a(sections, report)
    check_section_b(sections, report)
    check_section_c(sections, report)
    check_section_d(sections, report)
    check_section_e(sections, report)
    check_section_g(sections, report)
    check_section_h(sections, report)

    # Citations
    check_citation_format(text, report)
...
def main():
    ...
    parser.add_argument("--archetype", default="recruitment", help="Industry archetype (default: recruitment)")
