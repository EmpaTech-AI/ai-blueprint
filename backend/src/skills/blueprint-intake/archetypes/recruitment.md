...
The `compliance_deadline` column carries the specific regulatory or legal enforcement date
(`YYYY-MM-DD`) or `none`. A system migration date or internal project deadline does NOT qualify —
only a legally-mandated enforcement date. The blueprint-roadmap FB phase rule reads this field
as the primary machine-readable trigger for the compliance-deadline Now condition.

The `phase_dependency` column applies to Big Bet opportunities (those that reach `class=BigBet`
after D6 adjustment). `strict`: the "depends on any Next-assigned item → Later" rule is
