...
Before your output is passed to the orchestrator's quality gate (QG-2), you must run the
...
After producing the AI Readiness Snapshot, append the standardized Quality Self-Assessment
...
### Anti-Hallucination Rules for Maturity Scoring

Maturity scoring is especially vulnerable to hallucination because of the temptation to
"fill in" dimensions that have thin evidence. Special rules:

1. **No evidence = Early.** If a dimension has no supporting data from the dossier, score
   it as "Early" with [Insufficient Evidence — defaulted to Early]. Never score a dimension
   higher than Early without at least one specific data point.
2. **Conservative bias is mandatory.** When evidence is ambiguous between two adjacent levels,
   always score the lower level. State what would confirm the higher score.
3. **Pocket maturity ≠ organizational maturity.** One advanced team does not lift the org
   score. Note the pocket of excellence but score the organization at the lower level.
4. **Rationales must cite the dossier.** Every rationale must reference specific data from
   the Compressed Client Dossier (Section A, B, C, or D). General statements like "appears
   to be developing" without a citation are not acceptable.
5. **Upstream fidelity check.** Before producing your output, verify that any client facts
   you reference (revenue, headcount, industry, pain points) match the dossier exactly.
   Do not paraphrase into inaccuracy.

### Downstream Data Contract

Your output feeds blueprint-opportunities (Step 3) and blueprint-roadmap (Step 4). The
quality gate (QG-2) will verify these data contract requirements:

| Required Element | Where in Your Output | Criticality |
|-----------------|---------------------|-------------|
| All 6 dimensions scored (Early/Developing/Established) | Readiness Scorecard | Critical |
| Rationale per dimension with dossier citations | Dimension Rationales | Critical |
| Confidence level per dimension score | Readiness Scorecard | Critical |
| "What would change this score" per dimension | Dimension Rationales | Important |
| Overall pattern narrative | Overall Pattern | Critical |
| Key constraints for AI adoption | Key Constraints | Critical |
