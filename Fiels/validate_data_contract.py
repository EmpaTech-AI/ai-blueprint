...
USAGE
-----
    # Validate a skill output (Steps 1-5):
    ...
    # Pre-pipeline form quality check:
    python validate_data_contract.py --form-scores "3,2,1,3,2,1,2"

    # With industry-specific context:
    python validate_data_contract.py --skill opportunities --output map.md \\
        --documents "Revenue Report.xlsx" --industry "professional_services"
...
EVIDENCE GROUNDING FORMULA (V2)
---------------------------------
    HIGH = [Document-Backed] + [Form-Stated]
    LOW  = [Inferred] + [Assumption]
    UNTAGGED = estimated untagged claims (treated as [Assumption])

    Evidence Grounding % = HIGH / (HIGH + LOW + UNTAGGED) × 100

V1 omitted UNTAGGED from the denominator, which inflated scores when outputs
had many untagged claims. V2 corrects this.

HALLUCINATION RISK SCORING (V2 — 3-TIER MODEL)
------------------------------------------------
    Tier 1 Hard (phantom citations confirmed against doc list): -10% each
        → If any Tier 1 confirmed phantom exists, score capped at 50%
    Tier 2 Soft (false precision patterns): -5% each, max -25%
    Tier 3 Assumption Creep (untagged estimates): -3% each
    If no doc list provided: phantom citations score as Tier 3 (-3% each, no cap)

VERSION
-------
1.0 — 2026-05-18 — Initial release
2.0 — 2026-05-18 — V2 system alignment:
      - Fix Evidence Grounding formula (include UNTAGGED in denominator)
      - Fix Hallucination Risk (3-tier model with Tier 1 50% cap)
      - Add pre-pipeline form quality scoring mode
      - Add [Form:] citation exclusion (not a phantom)
      - Add new false precision patterns (causal fabrication, capability invention,
        industry assumption injection, forward projection, unqualified assumption)
      - Update REQUIRED_SECTIONS: opportunities adds "Additional Opportunities Note";
        assembly adds "Quality Self-Assessment"
      - Add assembly confidence-tag-leak detection
      - Add --industry flag for context-aware reporting
"""
...
# Source: methodology-and-contracts.md, quality-gate-algorithm.md, SKILL.md files
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PRE-PIPELINE FORM QUALITY SCORING
# Source: intake-form-quality-scoring.md
# ---------------------------------------------------------------------------

FORM_SECTION_NAMES: list[str] = [
...
# Industry-weighted multipliers: sections 1-4 weight 1.5×, sections 5-6 weight 1.0×,
# section 7 weights 0.75×
FORM_SECTION_WEIGHTS: list[float] = [1.5, 1.5, 1.5, 1.5, 1.0, 1.0, 0.75]

# Max possible weighted points: (4 × 3 × 1.5) + (2 × 3 × 1.0) + (1 × 3 × 0.75) = 18 + 6 + 2.25
FORM_MAX_WEIGHTED: float = 26.25

# Thresholds: (min_pct, label, action)
FORM_QUALITY_THRESHOLDS: list[tuple[float, str, str]] = [
    (80.0, "Excellent",    "PROCEED"),
    (65.0, "Adequate",     "PROCEED WITH CAUTION"),
    (50.0, "Marginal",     "HOLD — Request improved responses on low-scoring sections"),
    (0.0,  "Insufficient", "DO NOT PROCEED — Pipeline must not start until form quality improves"),
]

# Sections 1, 2, 3 (0-indexed: 0, 1, 2) are critical — if any score 0, block pipeline
FORM_CRITICAL_SECTION_INDICES: list[int] = [0, 1, 2]
...
FALSE_PRECISION_PATTERNS: list[tuple[str, str]] = [
    ...
    # Causal fabrication — "X causes Y" without evidence chain
    (r'\b(?:causes?|leads?\s+to|results?\s+in|drives?)\s+(?:a\s+)?\d+', "Causal claim with specific figure — verify causal chain and number are both evidenced"),
    # Industry assumption injection — "companies in this sector typically..."
    (r'\b(?:companies?|businesses?|firms?|organizations?)\s+(?:in\s+(?:this|your)\s+(?:sector|industry|space)|like\s+(?:yours?|this))\s+typically', "Industry generalization — verify this applies to this specific client, not just the sector"),
    # Capability invention — AI will achieve X without qualification
    (r'\bAI\s+(?:will|can|should)\s+(?:automate|replace|eliminate|handle|process)\s+\w+\s+(?:entirely|completely|fully|automatically)', "Unqualified AI capability claim — verify against client's actual maturity and tooling"),
    # Forward projection — "by [year], X will..."
    (r'\bby\s+(?:20\d{2}|next\s+(?:year|quarter))\s*[,.]?\s+\w+\s+(?:will|should|could)', "Forward projection — verify this is client-specific forecast, not generic industry trend"),
    # Unqualified assumption statements
    (r'\b(?:clearly|obviously|undoubtedly|certainly|definitely)\s+(?:has|have|is|are|will|shows?|indicates?)', "Unqualified certainty language — claims require evidence tags, not rhetorical certainty markers"),
]
...
# [Form: Section N] tags reference form sections, not documents — skip in phantom check
FORM_CITATION_PATTERN = re.compile(r'\[Form:\s*[^\]]+\]', re.IGNORECASE)
...
# All confidence tags as a single pattern — used to detect leakage in assembly output
ALL_CONFIDENCE_TAGS_PATTERN = re.compile(
...
@dataclass
class FormSectionResult:
    name: str
    score: int          # 0–3 raw
    weight: float       # multiplier
    weighted_score: float
    is_critical: bool   # sections 1–3 are critical


@dataclass
class FormQualityResults:
    section_results: list[FormSectionResult]
    raw_total: int
    ...
    label: str
    action: str
    critical_zero_sections: list[str]   # critical sections that scored 0
...
class ValidationResults:
    ...
    # Assembly-specific: confidence tags that leaked into the client-facing body
    assembly_confidence_tag_count: int = 0
...
def validate_form_quality(scores: list[int]) -> FormQualityResults:
    """
    Score the pre-pipeline intake form quality from 7 raw section scores (0–3 each).

    Parameters
    ----------
    scores : list[int]
        Seven integers in range [0, 3], one per form section in order:
        Company Fundamentals, Strategic Context, Operational Pain Points,
        Technology Landscape, People and Culture, Data Readiness, Budget and Timeline.

    Returns
    -------
    FormQualityResults
        Full breakdown including weighted total, percentage, label, action,
        and any pipeline-blocking conditions.
    """
    if len(scores) != 7:
        raise ValueError(f"Expected 7 form section scores, got {len(scores)}")
    for i, s in enumerate(scores):
        if not (0 <= s <= 3):
            raise ValueError(f"Score for section {i+1} must be 0–3, got {s}")
    ...
    for idx, (name, weight, raw) in enumerate(
        zip(FORM_SECTION_NAMES, FORM_SECTION_WEIGHTS, scores)
    ):
        ws = raw * weight
        is_crit = idx in FORM_CRITICAL_SECTION_INDICES
        section_results.append(FormSectionResult(
            name=name,
            score=raw,
            weight=weight,
            weighted_score=ws,
        ...
        weighted_total += ws
        raw_total += raw
        if is_crit and raw == 0:
            critical_zeros.append(name)

    pct = round((weighted_total / FORM_MAX_WEIGHTED) * 100.0, 1)

    label = "Insufficient"
    action = FORM_QUALITY_THRESHOLDS[-1][2]
    for threshold, lbl, act in FORM_QUALITY_THRESHOLDS:
        if pct >= threshold:
            label = lbl
            action = act
            break

    pipeline_blocked = bool(critical_zeros) or pct < 50.0

    return FormQualityResults(
        section_results=section_results,
        raw_total=raw_total,
        weighted_total=round(weighted_total, 2),
        max_weighted=FORM_MAX_WEIGHTED,
        percentage=pct,
        label=label,
        action=action,
        critical_zero_sections=critical_zeros,
        pipeline_blocked=pipeline_blocked,
    )


def generate_form_quality_report(fq: FormQualityResults) -> str:
    """Generate a formatted, color-coded report for form quality results."""
    lines: list[str] = []
    sep = colorize("─" * 70, Color.DIM)

    lines.append("")
    lines.append(sep)
    lines.append(bold(colorize("  AI VALUE BLUEPRINT — PRE-PIPELINE FORM QUALITY CHECK", Color.CYAN)))
    lines.append(sep)
    lines.append("")
    lines.append(bold("  SECTION SCORES"))
    lines.append("")

    for sr in fq.section_results:
        crit_marker = colorize(" [critical]", Color.DIM) if sr.is_critical else ""
        score_color = (Color.GREEN if sr.score == 3
                       else Color.AMBER if sr.score == 2
                       else Color.RED)
        score_str = colorize(f"{sr.score}/3", score_color)
        ws_str = f"(weighted: {sr.weighted_score:.2f})"
        lines.append(f"  {sr.name:<30} {score_str}  {ws_str}{crit_marker}")

    lines.append("")
    lines.append(f"  {'Weighted Total':<30} {fq.weighted_total:.2f} / {fq.max_weighted:.2f}")

    pct_color = (_score_color(fq.percentage) if fq.percentage >= 60
                 else Color.RED)
    lines.append(f"  {'Form Quality Score':<30} {colorize(f'{fq.percentage:.1f}%', pct_color)}")
    lines.append("")

    status_color = (Color.GREEN if fq.label == "Excellent"
                    else Color.AMBER if fq.label == "Adequate"
                    else Color.RED)
    lines.append(sep)
    lines.append(bold(f"  RESULT: {colorize(fq.label.upper(), status_color)}"))
    lines.append(bold(f"  ACTION: {colorize(fq.action, status_color)}"))
    lines.append(sep)

    if fq.critical_zero_sections:
        lines.append("")
        lines.append(colorize("  PIPELINE BLOCKED — Critical sections scored 0:", Color.RED))
        for sec in fq.critical_zero_sections:
            lines.append(colorize(f"    • {sec}", Color.RED))
        lines.append(colorize(
            "  Do not start the pipeline until these sections are completed.",
            Color.RED
        ))
    elif fq.pipeline_blocked:
        lines.append("")
        lines.append(colorize(
            "  PIPELINE BLOCKED — Overall score below 50%. Request improved form responses.",
            Color.RED
        ))
...
def _form_quality_to_dict(fq: FormQualityResults) -> dict:
    return {
        "metadata": {
        ...
        "weighted_total":          fq.weighted_total,
        "max_weighted":            fq.max_weighted,
        "percentage":              fq.percentage,
        "label":                   fq.label,
        "action":                  fq.action,
        "critical_zero_sections":  fq.critical_zero_sections,
        "pipeline_blocked":        fq.pipeline_blocked,
    }


def save_form_quality_json(fq: FormQualityResults, output_dir: str = ".") -> str:
    """Save form quality results as JSON. Returns path to saved file."""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    json_path = Path(output_dir) / f"form_quality_{ts}.json"
    json_path.write_text(
        json.dumps(_form_quality_to_dict(fq), indent=2, ensure_ascii=False),
        encoding="utf-8"
    )
...
def check_phantom_citations(
    ...
    [Form: Section N] tags are excluded — they reference form sections, not documents,
    and are valid citations in V2 outputs.

    If known_documents is provided, extracts document names from citation tags
    and cross-checks against the list (case-insensitive partial matching).
    ...
    for line_num, line in enumerate(lines, start=1):
        # Skip lines that are entirely [Form:...] citations
        if FORM_CITATION_PATTERN.fullmatch(line.strip()):
            continue
        ...
        for pattern_str in PHANTOM_CITATION_PATTERNS:
            ...
            for match in pattern.finditer(line):
                ...
                # Skip [Form: ...] matches embedded in longer lines
                if re.match(r'(?:section|form|intake form)\s*\d*', cited_raw, re.IGNORECASE):
                    continue
...
def calculate_evidence_grounding_score(
    tag_counts: TagCounts,
    untagged_estimate: int = 0,
) -> float:
    """
    Calculate the Evidence Grounding score as a percentage.

    Formula (V2 — quality-gate-algorithm.md, Dimension 1):
        HIGH = [Document-Backed] + [Form-Stated]
        LOW  = [Inferred] + [Assumption]
    ...
    V1 bug: denominator was HIGH + LOW only, omitting UNTAGGED. V2 includes
    UNTAGGED in the denominator so untagged outputs score lower correctly.

    Returns a float between 0.0 and 100.0.
    If no tags and no untagged estimate, returns 0.0 (flag as Red).
    """
    high  = tag_counts.high_confidence
    low   = tag_counts.low_confidence
    total = high + low + untagged_estimate

    if total == 0:
        return 0.0

    return round((high / total) * 100.0, 1)
...
def _calculate_completeness_score(
    ...
    critical_missing = sum(1 for s in sections if s.is_critical and not s.found)
...
def _calculate_hallucination_risk_score(
    ...
    """
    Estimate the Hallucination Risk dimension score using the V2 3-tier model.

    Scoring logic (evidence-grounding-checklist.md, Hallucination Spectrum):

        Start at 95% baseline.

        Tier 1 Hard (phantom citations, confirmed against doc list):
            -10% per confirmed phantom
            If ANY confirmed phantom exists: cap score at 50% (hard ceiling)

        Tier 2 Soft (false precision patterns):
            -5% per flag, maximum -25% total (5-flag cap)

        Tier 3 Assumption Creep (untagged claims):
            -3% per estimated untagged claim

        If no document list provided: phantoms treated as Tier 3 (-3% each, no cap).

        Floor: 0%.

    V1 bug: used flat -10%/-3% per phantom with no Tier 1 hard ceiling, no Tier 2
    max, and no Tier 3 untagged deduction. V2 corrects all three.

    Returns a float between 0.0 and 100.0.
    """
    score = 95.0
    has_confirmed_phantom = False

    for pc in phantom_citations:
        if known_documents_provided:
            has_confirmed_phantom = True
            score -= 10.0
        else:
            score -= 3.0    # Tier 3 treatment for unverifiable citations

    # Tier 1 hard ceiling — any confirmed phantom caps the score at 50%
    if has_confirmed_phantom:
        score = min(score, 50.0)

    # Tier 2 soft — false precision, capped at -25%
    tier2_deduction = min(len(false_precision_flags) * 5.0, 25.0)
    score -= tier2_deduction

    # Tier 3 — untagged claim creep
    score -= untagged_estimate * 3.0
...
def _estimate_untagged_claims(text: str, tag_counts: TagCounts) -> int:
    """
    Rough estimate of untagged claims by counting assertive sentences that
    ...
    Strategy:
        ...
        2. For each sentence, check if any confidence tag appears within a
           200-character window around the sentence
...
def _count_assembly_tag_leakage(text: str) -> int:
    """
    Count confidence tags present in the assembly output body (outside QSA block).

    Assembly is a client-facing document — confidence tags must be removed before
    delivery. Their presence in the body is a QG-FINAL failure condition.

    Returns the count of leaked tags (0 = clean).
    """
    # Strip the Quality Self-Assessment block — tags there are expected and correct
    body = re.sub(
        r'##\s+Quality\s+Self.?Assessment.*$', '', text,
        flags=re.IGNORECASE | re.DOTALL
    )
    return len(ALL_CONFIDENCE_TAGS_PATTERN.findall(body))
...
def generate_report(results: ValidationResults) -> str:
    ...
    The report covers:
        1. Header with file, skill, gate, industry, and timestamp
        2. Tag count summary
        3. Section presence check
        4. Assembly tag leakage check (assembly skill only)
        5. Hallucination red flags (phantoms + false precision)
        6. Quality Self-Assessment block check
        7. Dimension score estimates
        8. Estimated Confidence Score with band
        9. Gate decision
        10. Remediation action summary (if not Green)
    """
    lines: list[str] = []
    sep = colorize("─" * 70, Color.DIM)
    ...
    lines.append("")
    lines.append(sep)
    lines.append(bold(colorize("  AI VALUE BLUEPRINT — DATA CONTRACT VALIDATOR  v2.0", Color.CYAN)))
    lines.append(sep)
    ...
    if results.industry:
        lines.append(f"  {bold('Industry:')}  {results.industry}")
    ...
    # ── ASSEMBLY TAG LEAKAGE ─────────────────────────────────────────────────
    if results.skill == "assembly":
        lines.append("")
        lines.append(bold("  ASSEMBLY: CONFIDENCE TAG LEAKAGE CHECK"))
        lines.append("")
        if results.assembly_confidence_tag_count == 0:
            lines.append(colorize("  No confidence tags found in client-facing body. Clean.", Color.GREEN))
        else:
            lines.append(colorize(
                f"  FAIL: {results.assembly_confidence_tag_count} confidence tag(s) found in client-facing body.",
                Color.RED
            ))
            lines.append(colorize(
                "  All [Document-Backed], [Form-Stated], [Inferred], [Assumption], and",
                Color.RED
            ))
            lines.append(colorize(
                "  [Insufficient Evidence] tags must be removed before client delivery.",
                Color.RED
            ))
...
def _build_remediation_actions(
    ...
    if results.untagged_claim_estimate > 3:
        actions.append((
            "HIGH",
            f"Scan and tag ~{results.untagged_claim_estimate} estimated untagged claims — untagged counts against Evidence Grounding score",
            "Evidence Grounding (40%)"
        ))
    ...
    # Assembly-specific: tag leakage
    if results.skill == "assembly" and results.assembly_confidence_tag_count > 0:
        actions.append((
            "HIGH",
            f"Remove {results.assembly_confidence_tag_count} confidence tag(s) from client-facing body before delivery",
            "QG-FINAL Client Readiness"
        ))
    ...
    # Manual reviews always included
    actions.append((
        "MEDIUM",
...
def validate(
    ...
    industry : str
        Client industry identifier (for context in reports).
    ...
    # 5. Assembly-specific: check for confidence tag leakage in client-facing body
    if skill == "assembly":
        results.assembly_confidence_tag_count = _count_assembly_tag_leakage(text)

    # 6. Phantom citation check
    results.phantom_citations = check_phantom_citations(text, known_documents)

    # 7. False precision check
    results.false_precision_flags = detect_false_precision(text)

    # 8. Calculate Evidence Grounding score (V2: include untagged in denominator)
    results.evidence_grounding_score = calculate_evidence_grounding_score(
        results.tag_counts,
        untagged_estimate=results.untagged_claim_estimate,
    )
    ...
    # 10. Calculate Hallucination Risk score (V2: 3-tier model)
    ...
    # 11. Confidence Score (IC and DR use neutral defaults)
    ...
    # 13. Populate checks dict for JSON output
    results.checks = {
        ...
        "assembly_no_tag_leakage":
            (results.assembly_confidence_tag_count == 0) if skill == "assembly" else True,
    }

    # 14. Collect warnings
    if results.tag_counts.total_tagged == 0:
        results.warnings.append(
            "No confidence tags found — output may not follow tagging protocol."
        )
    if results.untagged_claim_estimate > 10:
        results.warnings.append(
            f"High untagged claim estimate ({results.untagged_claim_estimate}). "
            "Review output carefully — untagged claims reduce Evidence Grounding and Hallucination Risk scores."
        )
    if results.tag_counts.document_backed == 0 and results.tag_counts.form_stated == 0:
        results.warnings.append(
            "Zero high-confidence tags ([Document-Backed] + [Form-Stated] = 0). "
            "Evidence Grounding will score Red."
        )
    if skill == "assembly":
        if results.estimated_confidence_score < 90:
            results.warnings.append(
                "Assembly output must reach Green (90%+) for QG-FINAL. Current estimate is below threshold."
            )
        if results.assembly_confidence_tag_count > 0:
            results.warnings.append(
                f"{results.assembly_confidence_tag_count} confidence tag(s) leaked into client-facing body. "
                "Must be removed before client delivery — this is a QG-FINAL hard failure."
            )
...
def _results_to_dict(results: ValidationResults) -> dict:
    ...
    d: dict = {
        ...
        "dimension_notes": {
            ...
            "evidence_grounding":    "V2 formula: HIGH / (HIGH + LOW + UNTAGGED). Untagged estimate included in denominator.",
            "hallucination_risk":    "V2 3-tier model: Tier1 confirmed phantoms cap at 50%; Tier2 false precision max -25%; Tier3 untagged -3%/each.",
        },
    ...
    if results.skill == "assembly":
        d["assembly_confidence_tag_count"] = results.assembly_confidence_tag_count
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "AI Value Blueprint — Data Contract Validator v2.0\n"
            "Validates a skill output file against pipeline quality requirements.\n\n"
            "Also supports pre-pipeline form quality scoring (--form-scores).\n\n"
            "AI Assist BG | Blueprint Pipeline v2.0"
        ),
        ...
        epilog=(
            ...
            "  # With document list (enables confirmed phantom detection):\n"
            "  python validate_data_contract.py --skill maturity --output maturity_output.md \\\n"
            "      --documents 'Financial Report 2025.pdf,Org Chart.xlsx'\n\n"
            "  # Pre-pipeline form quality check (scores: 0-3 per section, 7 sections):\n"
            "  python validate_data_contract.py --form-scores '3,2,1,3,2,1,2'\n\n"
    ...
    # Skill-mode arguments
    parser.add_argument(
        "--skill",
        required=False,
        choices=list(REQUIRED_SECTIONS.keys()),
        help="The Blueprint skill that produced the output file (required unless --form-scores is used)",
    )
    parser.add_argument(
        "--output",
        required=False,
        metavar="PATH",
        help="Path to the skill's markdown output file to validate (required unless --form-scores is used)",
    )
    parser.add_argument(
        "--documents",
        metavar="DOC1,DOC2,...",
    ...
    parser.add_argument(
        ...
        metavar="INDUSTRY",
        default="",
        help="Client industry identifier for context-aware reporting (optional)",
    )

    # Pre-pipeline form quality mode
    parser.add_argument(
        "--form-scores",
        metavar="S1,S2,S3,S4,S5,S6,S7",
        default=None,
        help=(
            "Pre-pipeline mode: comma-separated scores (0–3) for each of the 7 intake "
            "form sections. Order: Company Fundamentals, Strategic Context, Operational "
            "Pain Points, Technology Landscape, People and Culture, Data Readiness, "
            "Budget and Timeline. Example: --form-scores '3,2,1,3,2,1,2'"
        ),
    )
...
if __name__ == "__main__":
    ...
    if args.form_scores:
        try:
            raw_scores = [int(s.strip()) for s in args.form_scores.split(",") if s.strip()]
        except ValueError:
            print(colorize("ERROR: --form-scores must be comma-separated integers (0–3).", Color.RED), file=sys.stderr)
            sys.exit(1)

        try:
            fq = validate_form_quality(raw_scores)
        except ValueError as e:
            print(colorize(f"ERROR: {e}", Color.RED), file=sys.stderr)
            sys.exit(1)

        if not args.json_only:
            report = generate_form_quality_report(fq)
            if args.no_color:
                report = _strip_ansi(report)
            print(report)

        # Save JSON alongside CWD
        json_path = save_form_quality_json(fq, output_dir=".")
        if not args.json_only:
            msg = f"  JSON report saved: {json_path}"
            print(colorize(msg, Color.DIM) if not args.no_color else msg)
            print()

        sys.exit(2 if fq.pipeline_blocked else 0)

    # ── SKILL VALIDATION MODE ─────────────────────────────────────────────
    if not args.skill or not args.output:
        print(
            colorize(
                "ERROR: --skill and --output are required unless --form-scores is used.",
                Color.RED
            ),
            file=sys.stderr,
        )
